...
export default function App() {
  ...
  const handleLogin = async (e) => {
    ...
    setLoading(true)
    const sSnap = await getDoc(doc(db,'horeb_students',id)) // HOREB students
    if(!sSnap.exists()){ setMessage("Invalid Horeb ID"); setLoading(false); return }
  ...
  // *** RADIO BUTTON VOTING UI FOR HOREB ***
  return (
            ...
            {candidates.filter(c=>c.position===pos).map(c=>(
              <label key={c.id} style={{display:'flex', alignItems:'center', gap:15, border: selections[pos]===c.id? '2px solid #0d5c5c' : '1px solid #ddd', padding:'12px 15px', borderRadius:10, cursor:'pointer', background: selections[pos]===c.id? '#f0ffff':'#fff'}}>
                <input
                  ...
                  onChange={()=>setSelections({...selections, [pos]:c.id})}
                  style={{width:22, height:22, accentColor:'#0d5c5c'}}
                />
